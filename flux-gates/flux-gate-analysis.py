        ...
        for k, exp in enumerate(reversed(experiments)):
            ...
            if "label_param_list" in list(kwargs.keys()):
                ...
                if has_observations:
                    ...
                    if legend == "regress":
                        label = "{:4.0f}m, r={:1.2f}".format(config["grid_dx_meters"], self.corr[id])
                    elif legend == "short":
                        label = "r={:1.2f}".format(self.corr[id])
                    else:
                        pass
...
class ExperimentDataset(Dataset):
    ...
    def __init__(self, id, *args, **kwargs):
        ...
        self.id = id
        self.config = dict()
        for v in ["pism_config", "run_stats"]:
...
def export_latex_table_flux(filename, flux_gates, params):
    ...
    f.write("\midrule \n")
    f.write("Total & {:2.1f}$\pm${:2.1f} & {:2.1f} \\\ \n".format(cum_flux, cum_flux_error, cum_length))
    f.write("\\bottomrule \n")
    f.write("\\end{tabular} \n")
    f.close
...
def export_latex_table_rmsd(filename, gate):
    ...
    f.write("\\toprule \n")
    f.write("\multicolumn{{2}}{{l}}{{{}}} & flux  & $\chi_{{g}}$ \\\ \n".format(unidecode(gate.gate_name)))
    ...
    f.write("observed && {:3.1f}$\pm${:2.1f} & {:2.2f} \\\ \n".format(observed_flux, observed_flux_error, error_norm))
    best_rmsd = gate.best_rmsd
    for k, val in enumerate(sorted(gate.p_ols, key=lambda x: gate.rmsd[x], reverse=False)):
        ...
        if k == 0:
            ...
            if gate.rmsd[val] < (error_norm + best_rmsd):
                ...
                f.write(
                    "Experiment {} & {} & {:2.1f}  & {:1.2f} \\\ \n".format(val, my_exp_str, my_flux, gate.rmsd[val])
                )
    f.write("\\bottomrule\n")
    f.write("\end{tabular}\n")
    f.close()
...
def export_latex_table_corr(filename, gate):
    ...
    f.write("\\begin{tabular} {l l cc }\n")
    f.write("\\toprule \n")
    f.write("\multicolumn{{2}}{{l}}{{{}}} &  $r$ & increase \\\ \n".format(unidecode(gate.gate_name)))
    f.write("&& (-)  & (\%)  \\\ \n")
    f.write("\midrule\n")
    best_corr = gate.best_corr
    for k, val in enumerate(sorted(gate.p_ols, key=lambda x: gate.corr[x], reverse=False)):
        config = gate.experiments[val].config
        my_flux = gate.experiment_fluxes[val]
        my_exp_str = ", ".join(
            [
...
def export_gate_table_rmsd(filename, exp):
    ...
    f.write(" ".join(["\\begin{tabular}", tab_str, "\n"]))
    f.write("\\toprule \n")
    f.write("Glacier & $\\bar U_{\\textrm{{obs,g}}}$ & $\chi_{\\textrm{{g}}}$ & $\\tilde \chi_{\\textrm{{g}}}$ \\\ \n")
...
def make_regression():
    ...
    for gate in flux_gates:
        ...
        ax.plot(grid_dx_meters, rmsd_data, dash_style, color="0.2", markeredgewidth=markeredgewidth)
    ...
    for id in (1, 11, 19, 23):
        ...
        if p_selected >= 0.05:
            line_l, = ax.plot(
                [grid_dx_meters[0], grid_dx_meters[-1]],
                bias_selected + np.array([grid_dx_meters[0], grid_dx_meters[-1]]) * trend_selected,
                linestyle="dashed",
                color=colorVal,
                linewidth=0.5,
            )
    ...
    if p_isbrae > 0.05:
        line_l, = ax.plot(
            [grid_dx_meters[0], grid_dx_meters[-1]],
            bias_isbrae + np.array([grid_dx_meters[0], grid_dx_meters[-1]]) * trend_isbrae,
            color="#8c510a",
            linewidth=1,
            linestyle="dashed",
        )
    ...
    ax.plot(grid_dx_meters, rmsd_data, dash_style, color="#8c510a", markeredgewidth=markeredgewidth)
    legend_handles.append(line_l)
    ...
    if p_ice_stream > 0.05:
        line_l, = ax.plot(
            [grid_dx_meters[0], grid_dx_meters[-1]],
            bias_ice_stream + np.array([grid_dx_meters[0], grid_dx_meters[-1]]) * trend_ice_stream,
            color="#01665e",
            linewidth=1,
            linestyle="dashed",
        )
    ...
    ax.plot(grid_dx_meters, rmsd_data, dash_style, color="#01665e", markeredgewidth=markeredgewidth)
    legend_handles.append(line_l)
    ...
    if p_global > 0.05:
        line_l, = ax.plot(
            [grid_dx_meters[0], grid_dx_meters[-1]],
            bias_global + np.array([grid_dx_meters[0], grid_dx_meters[-1]]) * trend_global,
            color="0.2",
            linewidth=1,
            linestyle="dashed",
        )
    ...
    ax.plot(grid_dx_meters, rmsd_data, dash_style, color="0.4", markeredgewidth=markeredgewidth)
    legend_handles.append(line_l)
    ...
    # plot errors
    ax.plot(grid_dx_meters, r2_data, dash_style, color="0.4", markeredgewidth=markeredgewidth)
    # print statistics
    ax.text(0.05, 0.7, "r$^\mathregular{{2}}$={:1.2f}".format(r2), transform=ax.transAxes)
    ax.set_xticks(grid_dx_meters)
    ax.set_xlabel("grid resolution (m)")
